...
# Funnktion für TF-IDF und LDA
def tfidf_und_lda(daten, tfidf_datei, feature_datei, lda_datei, themen_datei):
    vektorisierer = TfidfVectorizer()
    tfidf_matrix = vektorisierer.fit_transform(daten['review_text'])

    scipy.sparse.save_npz(tfidf_datei, tfidf_matrix)
    ...
    anzahl_themen = 15
    lda_modell = LatentDirichletAllocation(n_components=anzahl_themen, random_state=42)
    lda_matrix = lda_modell.fit_transform(tfidf_matrix)

    themen = []
    for i, thema in enumerate(lda_modell.components_):
        woerter_im_thema = vektorisierer.get_feature_names_out()[np.argsort(thema)][:-16:-1]
    ...
    np.save(lda_datei, lda_modell.components_)

    return vektorisierer, tfidf_matrix, themen

# Coherence Score
def kohärenz_berechnen(vektorisierer, tfidf_matrix, themen):
    ...
    score = kohärenz(themen, vektorisierer, tfidf_matrix)
...
def main():
    ...
    tfidf_datei = 'C:/temp/studium/TFIDF_tinder_reviews.npz'
    feature_datei = 'C:/temp/studium/TFIDF_features.txt'
    lda_datei = 'C:/temp/studium/LDA_themen.npy'
    themen_datei = 'C:/temp/studium/LDA_topics.txt'

    daten = daten_laden_und_filtern(dateipfad)
    daten = texte_bereinigen_und_lemmatisieren(daten)
    vektorisierer, tfidf_matrix, themen = tfidf_und_lda(daten, tfidf_datei, feature_datei, lda_datei, themen_datei)
    kohärenz_berechnen(vektorisierer, tfidf_matrix, themen)
